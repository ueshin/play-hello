...
object Application extends Controller with Defaults {
  ...
  def followings(limit: Int = 20, offset: Int = 0) = {
    currentUser match {
      case Some(currentUser) => {
        val(followings, users, cachedAt) = Cache.get(followingsCacheName(currentUser.email), "10min") {
        ...
        "@followings".asTemplate(followings.take(limit+offset).takeRight(limit), users, cachedAt)
      }
      case None => Action(Application.index())
    }
  }
